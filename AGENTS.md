...
按需更新：当前处于第 3 周（已完成第 1 周分支、第 2 周撤销）。

## 笔记与分支管理规则

- **`notes-dayN.md` 统一在 `main` 分支提交**（用户踩过坑：笔记散落在实验分支导致找不到）
- 实验/练习分支可随便建，但笔记必须回到 main 提交
- 每节课开始前检查当前分支，确保在 main 上写笔记
...
- **铁律：每天课程内容讲完立即直接补全完整笔记并 commit，不要等用户提醒、不要只留骨架**（用户明确要求：不写骨架，直接一课一文档写完整内容）
